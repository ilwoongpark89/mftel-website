...
const ConferenceTripView = memo(function ConferenceTripView({ items, onSave, onDelete, onReorder, currentUser }: { items: ConferenceTrip[]; onSave: (c: ConferenceTrip) => void; onDelete: (id: number) => void; onReorder: (list: ConferenceTrip[]) => void; currentUser: string }) {
    ...
    const handleFileDelete = async (id: number) => { if (!selected) return; const fl = (selected.files || []).find(x => x.id === id); if (fl?.url?.startsWith("https://")) { try { const tk = typeof window !== "undefined" ? localStorage.getItem("mftel-auth-token") || "" : ""; const res = await fetch("/api/dashboard-files", { method: "DELETE", body: JSON.stringify({ url: fl.url }), headers: { "Content-Type": "application/json", ...(tk ? { Authorization: `Bearer ${tk}` } : {} as Record<string, string>) } }); if (!res.ok) { alert("파일 삭제에 실패했습니다."); return; } } catch (e) { console.warn("파일 삭제 실패:", e); alert("파일 삭제에 실패했습니다."); return; } } const u = { ...selected, files: (selected.files || []).filter(x => x.id !== id) }; onSave(u); setSelected(u); };
...
const ResourceView = memo(function ResourceView({ resources, onSave, onDelete, onReorder, currentUser }: { resources: Resource[]; onSave: (r: Resource) => void; onDelete: (id: number) => void; onReorder: (list: Resource[]) => void; currentUser: string }) {
    ...
    const handleResFileDelete = async (id: number) => { if (!selectedRes) return; const fl = (selectedRes.files || []).find(x => x.id === id); if (fl?.url?.startsWith("https://")) { try { const tk = typeof window !== "undefined" ? localStorage.getItem("mftel-auth-token") || "" : ""; const res = await fetch("/api/dashboard-files", { method: "DELETE", body: JSON.stringify({ url: fl.url }), headers: { "Content-Type": "application/json", ...(tk ? { Authorization: `Bearer ${tk}` } : {} as Record<string, string>) } }); if (!res.ok) { alert("파일 삭제에 실패했습니다."); return; } } catch (e) { console.warn("파일 삭제 실패:", e); alert("파일 삭제에 실패했습니다."); return; } } const u = { ...selectedRes, files: (selectedRes.files || []).filter(x => x.id !== id) }; onSave(u); setSelectedRes(u); };
